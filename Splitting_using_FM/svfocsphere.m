function svfocsphere(nmech,mom,stype,vp,vs,dens,sv_pol,nodeflag,axflag,dataflag, r_displ)
%
%   function svfocsphere(m,stype,vp,vs,dens,sv_pol,nodeflag,axflag,dataflag)
%
...
I = (sv_pol(:,6) == -1);
sv_pol(I,2) = sv_pol(I,2)+180;

% converting P-wave amplitudes to polarities
sv_pol(:,1) = sv_pol(:,1)./abs(sv_pol(:,1));

%hsvfoc=figure('color','w','name','SV Wave Nodal Surfaces');
hold on;
daspect([1 1 1]);
%axis off;
...
for imech=1:nmech;
    ...
    if nodeflag == 1;
        ...
        svamp=svrad(az,inc,vp,vs,dens,eps,m, r_displ);
        %
        %  find the zero amplitude contor, if it exists
        %
        c=contourc(az,inc,svamp,[0 0]);
