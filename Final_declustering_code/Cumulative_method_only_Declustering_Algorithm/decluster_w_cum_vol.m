function [vol_sort, volrand_sort,V05, NV05_cat] = decluster_w_cum_vol(infile, PROB, simul_tag, print_clus_t0_file)
...
% 
% %% I want to exclude earthquakes with tetrahedra volume less 10^(-5)
% [xs_sort, ys_sort, zs_sort, vol_sort] = calc_tetvol(xs,ys,zs);
% 
% xs = xs_sort(vol_sort>=10^(-5));
% ys = ys_sort(vol_sort>=10^(-5));
% zs = zs_sort(vol_sort>=10^(-5));
% vol_sort = vol_sort(vol_sort>=10^(-5));
