...
while (ii  < 100) && (i< N_loop) 
    ...
    L=L_tmp; %W=W_tmp; 
    Strike=Strike_tmp;% Dip=Dip_tmp; % fault plane parameters

    splitfault_2D(Kfaults)
   ...
   % zt_tmp_i(:,:,i)=zt; hypocenter location in a cluster
    ...
    L_tmp_ii=L; %W_tmp_ii=W; 
    Strike_tmp_ii=Strike;
    ...
    if use_glo_var == 1
        ...
        JFINALL=Copy_of_faultcluster_2D(con_tol,Kfaults);
    end
    ...
    if (i < N_loop) && (minNt > 1)  %(minDip >= dip_threshold) && 
        ...
        L_tmp_i(:,:,i)=L_tmp_ii; %W_tmp_i(:,:,i)=W_tmp_ii; 
