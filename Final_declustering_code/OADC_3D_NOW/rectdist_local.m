function dmin=rectdist_local(xst,yst,zst,xvt,yvt,zvt,vec_plane_t,Ltn,Wtn,xbt,ybt,zbt,k,m)
...
u=[xst(k) yst(k) zst(k);xst(k) yst(k) zst(k);xst(k) yst(k) zst(k);xst(k) yst(k) zst(k)];
u1=[xst(k) yst(k) zst(k)];

% Construct vertices vector
v=[ xvt(m,1:4)' yvt(m,1:4)' zvt(m,1:4)'];
...
vec(1:3)=vec_plane_t(m,1:3);
u2=u1-[xbt(m) ybt(m) zbt(m)];
