function beachball_CERI_code_3D(fm, centerX, centerY, centerZ, diam, ta, color)
% originally function bb(fm, centerX, centerY, diam, ta, color) by Dr.
% Oliver Boyd
%
% To run, 
% clear all; close all ;clc;
% 
% beachball_CERI_code_3D([0 45 90; 0 50 0; 0 30 30],[0 5 -5],[0 5 -5],[10 5 -5],[1 1 1],0,'k');
% xlim([-20 20]);
% ylim([-20 20]);
% axis equal
% grid on
...
%	centerZ - place beachball(s) at position centerZ
...
%
% Edits by Seun Fadugba
% Fixed bugs to plot more than one focal mechanisms. 8/26/2019
% Seun extended the code to plot FM in 3D but the FM is still 2D. 8/27/2019
...
% Fixed bugs by Seun Fadugba to plot more than one focal mechanisms
% if ne > 1
...
for ev = 1:ne
    ...
    if D > 0
       ...
       xx = x*D/90 + CX;
       yy = ampy*y*D/90 + CY;
       zz = ones(1,length(xx))*CZ;
       Z = ones(1,length(X))*CZ;
       if ta == 0
          fill3(xx,yy,zz,'w','FaceAlpha',0.2)
          fill3(Y,X,Z,color,'FaceAlpha',1)%,'FaceColor',[0.5 0.5 0.5])
          line(xx,yy,zz,'color','k','linewidth',0.2);
       else
          fill3m(yy,xx,zz,'w','FaceAlpha',0.2)
          fill3m(X,Y,Z,color,'FaceAlpha',1)%,'FaceColor',[0.5 0.5 0.5])
          linem(yy,xx,zz,'color','k','linewidth',0.2);
       end
    else
       if ta == 0
          fill3(X,Y,Z,color,'FaceAlpha',1)%,'FaceColor',[0.5 0.5 0.5])
       else
          fill3m(Y,X,Z,color,'FaceAlpha',1)%,'FaceColor',[0.5 0.5 0.5])
       end
       view(90,-90)
    end
