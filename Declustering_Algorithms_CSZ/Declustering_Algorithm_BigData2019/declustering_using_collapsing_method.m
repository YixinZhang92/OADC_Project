...
infile = 'declustered_hypo_0.6.txt'; orig_cdf_max = 0.6;

cdf_max = 0.6; % to decluster the collapsed earthquakes
...
[x_colap, y_colap, z_colap] = calc_collapsed_cloud(xs, ys, zs, 1.5, 3);
...
% ylabel('Latitude (km)','FontSize',18)
% zlabel('depth (km)','FontSize',18)
% title(['Original Declustered Earthquake Distribution (cdf = ' num2str(orig_cdf_max) ')'],'FontSize',18); 
% 
% hold on;
% ax2 = subplot (2,2,2);
% scatter3(x_colap,y_colap,z_colap,'filled','MarkerEdgeColor','k',...
...
% ylabel('Latitude (km)','FontSize',18)
% zlabel('depth (km)','FontSize',18)
% title(['Declustered Collapsed Earthquakes (cdf max = ' num2str(cdf_max) ')'],'FontSize',18); 
% 
% 
% Link = linkprop([ax1, ax2, ax3, ax4], {'CameraUpVector', 'CameraPosition', 'CameraTarget'});
% setappdata(gcf, 'StoreTheLink', Link);
